...
pdf(file="06.TSNE_Mycell.pdf",width=8,height=6)
...
dev.off()
###根据marker基因进行细胞类型注释
features<-c("CD163","ITGAM","CD4","ITGAX","THBD","IL3RA","NRP1","LILRA4")
class<-c("Macrophage","Macrophage","cDC","cDC","cDC","pDC","pDC","pDC")
p1<-DotPlot(scRNAsub.T,features = features,cols = colorRampPalette(colors = c(mypal))(3),group.by = "cellType_My")
pdot<-p1$data
pdot$class<-rep(class,3)
pdot$features.plot<-factor(pdot$features.plot,levels = unique(pdot$features.plot))
pdot$class<-factor(pdot$class,levels = c("Macrophage","cDC","pDC"))
pdot$id<-factor(pdot$id,levels = c("Macrophage","cDC","pDC "))
...
pdot<-ggplot(pdot,aes(x=features.plot,y=id))+
  ...
  theme(panel.grid = element_blank(),
        ...
        axis.text.x = element_text(angle = 90,hjust = 1,vjust = 0.5,face = 'bold',size = 15),
        axis.text.y = element_text(angle = 0,hjust = 1,vjust = 0.5,face = 'bold',size = 15))+
  coord_cartesian(clip = 'off')
pdot_test<-pdot+
  scale_x_discrete(position = 'top')+
  theme(axis.text.x = element_text(angle = 90,hjust = 0,size = 15))
annoRect(object = pdot_test,annoPos = "top",aesGroup = T,aesGroName = 'class',yPosition = c(3.6,5),rectWidth = 1,
         rectAngle = 90,normRectShift = 0,textHVjust = 1,textRot = 45,hjust = 0,textShift = 0,
         rotateRect = T,alpha = 0.5,
         addText = T)
ggsave("cell_bubble_My_celltype.pdf",width = 10,height =8)
###
scRNAsub.T@meta.data$cellType_My<-ifelse(scRNAsub.T@meta.data$seurat_clusters%in%c(6),"pDC ",
                                    ifelse(scRNAsub.T@meta.data$seurat_clusters%in%c(5,9,13,14,15,17,18),"cDC","Macrophage"))
#先画一个，获取图例
pdf(file="06.TSNE_Mycell_cellType.pdf",width=8,height=6)
DimPlot(object = scRNAsub.T, label = T,pt.size = 1,label.size =6,label.box = T,repel =T,group.by = "cellType_My",
        cols = colorRampPalette(colors = c(mypal))(3))+NoLegend()+
  labs(x="tSNE1",y="tSNE2",title = "")+
  theme(axis.text.x = element_text(angle = 0, hjust = 1,size = 15,face = "bold"), 
        axis.text.y = element_text(angle = 0, size = 15,face = "bold"),
        axis.title.y = element_text(angle = 90, size = 20,face = "bold"),
...
dev.off()
#按照细胞类型做差异分析
Idents(scRNAsub.T)="cellType_My"
skcm.markers_celltype.My<- FindAllMarkers(object = scRNAsub.T,
                                         only.pos = FALSE,
                                         min.pct = 0.25,
                                         logfc.threshold = 0)

top = 50 # can be adjusted as needed
TopMarkers <- skcm.markers_celltype.My %>% filter(p_val_adj < 0.01 & abs(avg_log2FC)>0.5) %>% group_by(cluster) %>%
...
bar<-bar[,c(8,9,14)]
bar<-reshape2::dcast(bar,HT+group~cellType_My,fill = 0)
bar<-melt(bar)
...
ggplot(data = bar,aes(x=HT,y=value))+
  ...
  theme_bw()+scale_fill_manual(values =c(colorRampPalette(colors = c(jdb_palette("lawhoops")))(3)))+
...
ggsave("bar_My_all.pdf",width =6,height = 6)
library(rstatix)
library(SeuratDisk)
cellinfo <- Seurat::FetchData(scRNAsub.T, vars = c("HT", "group","cellType_My"))
#cellinfo<-cellinfo[which(cellinfo$group=="metastasis"),]
cell.col <- setNames(object = c(mypal[5], mypal[6]),
                     nm = c("HT", "Non-HT"))

# 构建seurat_clusters和Group的列联表
tbl <- table(cellinfo$cellType_My, cellinfo$HT)
tbl
...
p2
ggsave(filename = "Mybar_sig.pdf", width = 8, height = 6)
#通讯分析
library(Seurat)
library(tidyverse)
library(iTALK)
...
##==细胞通讯关系概览==##
cell.meta <- subset(ht8@meta.data, select=c("orig.ident","cellType_My"))
...
plot2 <- plot_cell_trajectory(mycds, color_by = "cellType_My")+
  scale_colour_manual(
    values = cols_1
    # aesthetics = c("colour", "fill")
  )
ggsave("celltype_My.pdf", plot = plot2, width = 6, height = 5)
##Pseudotime轨迹图
plot3 <- plot_cell_trajectory(mycds, color_by = "Pseudotime")
ggsave("Pseudotime.pdf", plot = plot3, width = 6, height = 5)

p2 <- plot_cell_trajectory(mycds, color_by = "cellType_My")+NoLegend()+
  scale_colour_manual(
    values = cols_1
    # aesthetics = c("colour", "fill")
  ) + facet_wrap(~cellType_My, nrow =1)

ggsave("trajectory_facet.pdf", plot = p2, width = 6, height =3)
...
sig_diff.genes <- TopMarkers[which(TopMarkers$cluster%in%c("Macrophage")),"gene"]#用我们关注的亚群的marker基因
...
plot_genes_branched_heatmap(mycds_sub_beam,  branch_point = 1, num_clusters =4,show_rownames = T)
dev.off()
...
plot_genes_branched_heatmap(mycds_sub_beam,  branch_point = 3, num_clusters = 6, show_rownames = T)
dev.off()
...
a <- subset(scRNAsub.T@meta.data, cellType_My=="cDC")
a<- row.names(a)
write.table(a,"cDC.txt",col.names = F,row.names = F,sep = "\t",quote = F)
b<-TopMarkers[which(TopMarkers$cluster=="cDC"),"gene"]
b<-b$gene
write.table(b,"cDC_marker.txt",col.names = F,row.names = F,sep = "\t",quote = F)
