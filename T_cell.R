...
dev.off()
###根据marker基因进行细胞类型注释
features<-c("CD4","CD8A","CD8B","FOXP3","CTLA4","IL2RA","NCR1","NCAM1","CD28",
            "CD69","ICOS","TNFRSF4","TNFRSF9","CD27","IL2","TNF","IFNG","PDCD1","BTLA","KIR3DL1","LAG3","HAVCR2",
            "ADORA2A","HAVCR1","SELL","CCR7")
class<-c("CD4 T cells","CD8 T cells","CD8 T cells","Regulatory T cells","Regulatory T cells","Regulatory T cells",
         "NKT cells","NKT cells","Activated T cells","Activated T cells","Activated T cells","Activated T cells",
         "Activated T cells","Activated T cells","Activated T cells","Activated T cells",
         "Activated T cells","Exhausted T cells","Exhausted T cells","Exhausted T cells","Exhausted T cells",
         "Exhausted T cells","Exhausted T cells","Exhausted T cells","Memory T cells","Memory T cells")
###根据报告的文献进行注释
features<-c("CCR7","IL7R","KLF2","SELL","FOXP3","CTLA4","IL2RA","TIGIT",
            "CXCL13","HAVCR2","LAG3",
            "GZMK","GZMH","NKG7","EOMES",
            "LMNA","ANXA1","IFNG",
            "GZMB","PRF1",
            "GNLY","KLRD1","FCER1G")
class<-c("CD4 Tn","CD4 Tn","CD4 Tn","CD4 Tn","CD4 Treg","CD4 Treg","CD4 Treg","CD4 Treg",
         "CD8 Tex","CD8 Tex","CD8 Tex",
         "CD8 Teff","CD8 Teff","CD8 Teff","CD8 Teff",
         "CD8 Tem","CD8 Tem","CD8 Tem",
         "NKT","NKT","NK","NK","NK")

p1<-DotPlot(scRNAsub.T,features = features,cols = colorRampPalette(colors = c(mypal))(9),group.by = "cellType_T")
pdot<-p1$data
pdot$class<-rep(class,9)
...
pdot<-ggplot(pdot,aes(x=features.plot,y=id))+
  ...
  scale_size(range = c(0.2,10),name='Percentage')+
...
annoRect(object = pdot_test,annoPos = "top",aesGroup = T,aesGroName = 'class',yPosition = c(9.6,12),rectWidth = 1,
         rectAngle = 90,normRectShift = 0.1,textHVjust = 1.5,textRot = 45,hjust = 0,textShift = 0,
         rotateRect = T,alpha = 0.5,
         addText = T)
ggsave("cell_bubble_T_celltype.pdf",width = 10,height =8)
###
scRNAsub.T@meta.data$cellType_T<-ifelse(scRNAsub.T@meta.data$seurat_clusters%in%c(0,1,2,5,6,7,9,11,18,23),"CD4_Tn ",
                                        ifelse(scRNAsub.T@meta.data$seurat_clusters%in%c(17,21),"CD4_Treg",
                                               ifelse(scRNAsub.T@meta.data$seurat_clusters%in%c(19),"CD4_Tn_Treg",
                                                      ifelse(scRNAsub.T@meta.data$seurat_clusters%in%c(24),"CD8_Tex",
                                                             ifelse(scRNAsub.T@meta.data$seurat_clusters%in%c(14),"CD8_Tex_Teff",
                                                                    ifelse(scRNAsub.T@meta.data$seurat_clusters%in%c(8,10,12),"CD8_Teff",
                                                                           ifelse(scRNAsub.T@meta.data$seurat_clusters%in%c(3,4),"CD8_Teff_Tem",
                                                                                  ifelse(scRNAsub.T@meta.data$seurat_clusters%in%c(13),"CD8_Tem","NKT_NK"))))))))
#先画一个，获取图例
pdf(file="06.TSNE_Tcell_cellType.pdf",width=8,height=6)
DimPlot(object = scRNAsub.T, label = T,pt.size = 1,label.size =6,label.box = T,repel =T,group.by = "cellType_T",
        cols = colorRampPalette(colors = c(mypal))(9))+NoLegend()+
  labs(x="tSNE1",y="tSNE2",title = "")+
  theme(axis.text.x = element_text(angle = 0, hjust = 1,size = 15,face = "bold"), 
        axis.text.y = element_text(angle = 0, size = 15,face = "bold"),
        axis.title.y = element_text(angle = 90, size = 20,face = "bold"),
...
dev.off()
#按照细胞类型做差异分析
Idents(scRNAsub.T)="cellType_T"
skcm.markers_celltype.T<- FindAllMarkers(object = scRNAsub.T,
                                          only.pos = FALSE,
                                          min.pct = 0.25,
                                          logfc.threshold = 0)

top = 50 # can be adjusted as needed
TopMarkers <- skcm.markers_celltype.T %>% filter(p_val_adj < 0.01 & abs(avg_log2FC)>0.5) %>% group_by(cluster) %>%
...
bar<-bar[,c(8,9,11)]
bar<-reshape2::dcast(bar,HT+group~cellType_T,fill = 0)
bar<-melt(bar)
...
ggplot(data = bar,aes(x=HT,y=value))+
  ...
  theme_bw()+scale_fill_manual(values =c(colorRampPalette(colors = c(jdb_palette("lawhoops")))(9)))+
...
ggsave("bar_T_all.pdf",width =6,height = 6)
library(rstatix)
library(SeuratDisk)
cellinfo <- Seurat::FetchData(scRNAsub.T, vars = c("HT", "group","cellType_T"))
#cellinfo<-cellinfo[which(cellinfo$group=="metastasis"),]
cell.col <- setNames(object = c(mypal[6], mypal[9]),
                     nm = c("HT", "Non-HT"))

# 构建seurat_clusters和Group的列联表
tbl <- table(cellinfo$cellType_T, cellinfo$HT)
tbl
...
p2
ggsave(filename = "Tbar_sig.pdf", width = 8, height = 6)
save(scRNAsub.T,file = "T_recluster.Rdata")
load("T_recluster.Rdata")
#通讯分析
library(Seurat)
library(tidyverse)
library(iTALK)
...
##==细胞通讯关系概览==##
cell.meta <- subset(nht9@meta.data, select=c("orig.ident","cellType_T"))
...
plot2 <- plot_cell_trajectory(mycds, color_by = "cellType_T")+
  scale_colour_manual(
    values = cols_1
    # aesthetics = c("colour", "fill")
  )
ggsave("celltype_T.pdf", plot = plot2, width = 6, height = 5)
##Pseudotime轨迹图
plot3 <- plot_cell_trajectory(mycds, color_by = "Pseudotime")
ggsave("Pseudotime.pdf", plot = plot3, width = 6, height = 5)

p2 <- plot_cell_trajectory(mycds, color_by = "cellType_T")+NoLegend()+
  scale_colour_manual(
    values = cols_1
    # aesthetics = c("colour", "fill")
  ) + facet_wrap(~cellType_T, nrow =2)
...
sig_diff.genes <- TopMarkers[which(TopMarkers$cluster%in%c("CD8_Tex")),"gene"]#用我们关注的亚群的marker基因
...
a <- subset(scRNAsub.T@meta.data, cellType_T=="CD4_Tn_Treg")
a<- row.names(a)
write.table(a,"CD4_Tn_Treg.txt",col.names = F,row.names = F,sep = "\t",quote = F)
b<-TopMarkers[which(TopMarkers$cluster=="CD4_Tn_Treg"),"gene"]
b<-b$gene
write.table(b,"CD4_Tn_Treg_marker.txt",col.names = F,row.names = F,sep = "\t",quote = F)

a <- subset(scRNAsub.T@meta.data, cellType_T=="CD8_Tex_Teff")
a<- row.names(a)
write.table(a,"CD8_Tex_Teff.txt",col.names = F,row.names = F,sep = "\t",quote = F)
b<-TopMarkers[which(TopMarkers$cluster=="CD8_Tex_Teff"),"gene"]
b<-b$gene
write.table(b,"CD8_Tex_Teff_marker.txt",col.names = F,row.names = F,sep = "\t",quote = F)
